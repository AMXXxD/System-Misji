...
#define JAILBREAK  // Zakomentuj jeśli nie chcesz misji związanych z modem jailbreak
...
new opis_misji[][] =
{
	...
	"Rozegraj %d rund",
	"Zabij %d osób",
	"Zabij %d osób w głowę",
	"Zadaj %d dmg",
	"Zadaj %d dmg w glowe",
	"Podloz/Rozbroj C4 %d razy",
	"Podnies %d amunicji",
	"Przetrwaj %d rund bez zgonu",
	"Zabij %d osób bez zgonu",
	"Zabij %d osób w głowę bez zgonu",
	"Zadaj %d dmg bez zgonu",
	"Zadaj %d dmg w głowę bez zgonu",
	"-",
	"-",
	"-",
	"-",
	...
	"-",
	"-",
	"Zabij %d prowadzących",
	"Zabij %d prowadzących w głowę",
	"Zabij %d strażników",
	"Zabij %d strażników w głowę",
	"Zadaj %d dmg prowadzącemu",
	"Zadaj %d dmg prowadzącemu w głowę",
	"Zadaj %d dmg strażnikom",
	"Zadaj %d dmg strażnikom w głowę",
	"Przetrwaj %d razy jako ostatni więźien",
	"Zdobądź %d życzeń"
}
...
{
    precache_sound("AS/MISJE/misja_ukonczona.mp3");
}
...
public plugin_natives()
{
	...
	register_native("as_get_user_wyk_misje", "natyw_pobierz_wykonane_misje", 1);
	register_native("as_pomin_misje", "natyw_pomin_misje", 1);
...
public natyw_pomin_misje(id)
{
	...
	client_cmd(id, "mp3 play sound/AS/MISJE/misja_ukonczona.mp3");
	set_dhudmessage(70, 255, 0, -1.0, -1.0, 2, 0.05, 3.0, 0.05, 1.0)
	show_dhudmessage(id, "!!! Mission Passed !!!^n+%d szlugow^nWpisz /m lub /misje, aby rozpoczac nowa misje!", nagroda_misji_gracza[id]);
	
	cs_set_user_money(id, cs_get_user_money(id) + nagroda_misji_gracza[id]);
}
...
public MyFile_INIT()
{
	...
	format(plik_akty, 127, "%s/AS/MISJE/AKTY.ini", configs_dir);
	format(plik_misje, 127, "%s/AS/MISJE/MISJE.ini", configs_dir);
	format(plik_natywy, 127, "%s/scripting/include/AS/MISJE.inc", base_dir);
	
		
	if(!dir_exists("addons/amxmodx/configs/AS"))
		mkdir("addons/amxmodx/configs/AS");
		
	if(!dir_exists("addons/amxmodx/configs/AS/MISJE"))
		mkdir("addons/amxmodx/configs/AS/MISJE");
	
	if(!dir_exists("addons/amxmodx/scripting/include/AS"))
		mkdir("addons/amxmodx/scripting/include/AS");
	...
	if(!file_exists(plik_natywy))
	{
		...
		write_file(plik_natywy, "native as_get_user_wyk_misje(id)               // Pobiera ilosc wykonanych misji gracza", -1);
		write_file(plik_natywy, "native as_pomin_misje(id)                      // Pomija aktualnie wybrana misje gracza", -1);
	}
	...
	if(!file_exists(plik_misje))
	{
		...
		#if defined JAILBREAK
		write_file(plik_misje, "// 20 - Zabij x prowadzacych", -1);
		write_file(plik_misje, "// 21 - Zabij x prowadzacych w glowe", -1);
		write_file(plik_misje, "// 22 - Zabij x straznikow", -1);
		write_file(plik_misje, "// 23 - Zabij x straznikow w glowe", -1);
		write_file(plik_misje, "// 24 - Zadaj x dmg prowadzacemu", -1);
		write_file(plik_misje, "// 25 - Zadaj x dmg prowadzacemu w glowe", -1);
		write_file(plik_misje, "// 26 - Zadaj x dmg straznikom", -1);
		write_file(plik_misje, "// 27 - Zadaj x dmg straznikom w glowe", -1);
		write_file(plik_misje, "// 28 - Przetrwaj x razy jako ostatni wiezien", -1);
		write_file(plik_misje, "// 29 - Zdobadz x zyczen", -1);
		#endif
		write_file(plik_misje, "//", -1);
...
public Menu_Glowne(id)
{
		...
		for(new i = 0, n = 1; read_file(plik_misje, i, LINIA_Z_PLIKU,127, Dlugosc); i++)
		{
			...
			if(containi(LINIA_Z_PLIKU, "//") == -1 && !equal(LINIA_Z_PLIKU, "", 1) && wybrany_akt[id] == str_to_num(DANE[0]))
			{
				if(n == top15_dane[id][id_misji_gracza])
				{
					...
					{
						format(tytul,155, "\r[\y%s\r]^n^n\wCel misji: \r%s^n\wWymagana broń: \r%s^n\wNagroda: \r%d szlugów", DANE[1], opis_misji_item, bronie_misji[str_to_num(DANE[5])], str_to_num(DANE[4]));
						format(item1,63, "\d%s z broni %s, aby ukonczyc misje!", opis_misji_item, bronie_misji[str_to_num(DANE[5])]);
					}
					else
					{
						format(tytul,155, "\r[\y%s\r]^n^n\wCel misji: \r%s^n\wNagroda: \r%d szlugów", DANE[1], opis_misji_item, str_to_num(DANE[4]));
	...
	{
		menu = menu_create("\d-\w=\y>\r MISJE\y <\w=\d-", "Menu_Glowne_Handle");
...
public Menu_Glowne_Handle(id, menu, item)
{
	...
	switch(item)
	{
		case 0:
		{
			new menu2 = menu_create("\d-\w=\y>\r MISJE\y <\w=\d-", "Menu_Glowne_Handle2");
			...
			menu_setprop(menu2, MPROP_BACKNAME, "Wroc");
...
public Menu_Misje_Handle(id, menu, item)
{
	...
	{
		client_print_color(id, id, "%s Wybrana misja jest juz^3 zaliczona!", PREFIX);
		menu_destroy(menu);
		return PLUGIN_CONTINUE;
	}
	else if(item + 1 < top15_dane[id][wykonanych_misji_w_akcie] + 1)
...
public Zabojstwo()
{
	...
	new CsTeams: team = cs_get_user_team(ofiara);
		
	if(bron_misji_gracza[zabojca] > 0)
	{
		#if defined JAILBREAK
		if(typ_misji_gracza[zabojca] == 20 && ofiara == jail_get_prowadzacy() && bron == bron_misji_gracza[zabojca])
			top15_dane[zabojca][progres_misji_garcza]++;
			
		if(typ_misji_gracza[zabojca] == 21 && ofiara == jail_get_prowadzacy() && hs && bron == bron_misji_gracza[zabojca])
			top15_dane[zabojca][progres_misji_garcza]++;
			
		if(typ_misji_gracza[zabojca] == 22 && team == CS_TEAM_CT && ofiara != jail_get_prowadzacy() && bron == bron_misji_gracza[zabojca])
			top15_dane[zabojca][progres_misji_garcza]++;
			
		if(typ_misji_gracza[zabojca] == 23 && team == CS_TEAM_CT && ofiara != jail_get_prowadzacy() && hs && bron == bron_misji_gracza[zabojca])
			top15_dane[zabojca][progres_misji_garcza]++;
		#endif
	...
	{
		#if defined JAILBREAK
		if(typ_misji_gracza[zabojca] == 20 && ofiara == jail_get_prowadzacy())
			top15_dane[zabojca][progres_misji_garcza]++;
			
		if(typ_misji_gracza[zabojca] == 21 && ofiara == jail_get_prowadzacy() && hs)
			top15_dane[zabojca][progres_misji_garcza]++;
			
		if(typ_misji_gracza[zabojca] == 22 && team == CS_TEAM_CT && ofiara != jail_get_prowadzacy())
			top15_dane[zabojca][progres_misji_garcza]++;
			
		if(typ_misji_gracza[zabojca] == 23 && team == CS_TEAM_CT && ofiara != jail_get_prowadzacy() && hs)
			top15_dane[zabojca][progres_misji_garcza]++;
		#endif
	...
	if(typ_misji_gracza[ofiara] == 8 || typ_misji_gracza[ofiara] == 9 || typ_misji_gracza[ofiara] == 10 || typ_misji_gracza[ofiara] == 11 || typ_misji_gracza[ofiara] == 12)
	{
		...
		client_print_color(ofiara, ofiara, "%s Zaliczyles^4 zgon!^1 Postep misji zostal^3 wyzerowany! :(", PREFIX);
	}
...
public Obrazenia(victim, inflictor, attacker, Float:damage, damagebits)
{
	...
	{
		#if defined JAILBREAK
		if(typ_misji_gracza[attacker] == 24 && victim == jail_get_prowadzacy() && bron == bron_misji_gracza[attacker])
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
			
		if(typ_misji_gracza[attacker] == 25 && victim == jail_get_prowadzacy() && hs && bron == bron_misji_gracza[attacker])
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
			
		if(typ_misji_gracza[attacker] == 26 && team == CS_TEAM_CT && victim != jail_get_prowadzacy() && bron == bron_misji_gracza[attacker])
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
			
		if(typ_misji_gracza[attacker] == 27 && team == CS_TEAM_CT && victim != jail_get_prowadzacy() && hs && bron == bron_misji_gracza[attacker])
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
		#endif
	...
	{
		#if defined JAILBREAK
		if(typ_misji_gracza[attacker] == 24 && victim == jail_get_prowadzacy())
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
			
		if(typ_misji_gracza[attacker] == 25 && victim == jail_get_prowadzacy() && hs)
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
			
		if(typ_misji_gracza[attacker] == 26 && team == CS_TEAM_CT && victim != jail_get_prowadzacy())
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
			
		if(typ_misji_gracza[attacker] == 27 && team == CS_TEAM_CT && victim != jail_get_prowadzacy() && hs)
			top15_dane[attacker][progres_misji_garcza] += floatround(damage, floatround_round);
		#endif
...
{
	#if defined JAILBREAK
	if(typ_misji_gracza[id] == 28 && id == jail_get_prisoner_last())
		top15_dane[id][progres_misji_garcza]++;
	#endif
...
{	
	...
	for(new id = 1; id < 33; id++)
	{
		...
		if(top15_dane[id][id_misji_gracza] > 0)
			format(status,127, "Misja: %s | Postep: [%d/%d] | Wyk: [%d]", opis_misji_status[id], top15_dane[id][progres_misji_garcza], cel_misji_gracza[id], top15_dane[id][wykonanych_misji]);
		else
			format(status,127, "Wpisz /m lub /misje, aby rozpocząć misję i wygrać nagrody!");
